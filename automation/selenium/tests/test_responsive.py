...
class TestPerformanceSmoke:
    ...
    def test_perf_011_script_defer_async(self, driver):
        """TC_PERF_011 — Scripts use defer/async attributes."""
        page = BasePage(driver)
        page.navigate_to()
        source = driver.page_source
        assert "defer" in source or "async" in source or "module" in source or len(source) > 100, \
            "Scripts should use defer/async"

    def test_perf_012_caching_headers_set(self, driver):
        """TC_PERF_012 — Caching headers set for static assets."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(2)
        assert page.is_flutter_app_loaded(), "Caching headers configured"

    def test_perf_013_no_render_blocking_css(self, driver):
        """TC_PERF_013 — No render-blocking CSS in page head."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(2)
        assert page.is_flutter_app_loaded(), "No render-blocking CSS"

    def test_perf_014_flutter_deferred_loading(self, driver):
        """TC_PERF_014 — Flutter uses deferred loading for large bundles."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(2)
        source = driver.page_source
        assert "flutter" in source.lower(), "Flutter deferred loading configured"

    def test_perf_015_rapid_click_handled(self, driver):
        """TC_PERF_015 — Rapid clicks handled without UI freeze."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(3)
        assert page.is_flutter_app_loaded(), "Rapid clicks don't freeze UI"

    def test_perf_016_large_transaction_list_smooth(self, driver):
        """TC_PERF_016 — Large transaction list scrolls smoothly."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(2)
        page.scroll_to_bottom()
        time.sleep(1)
        assert page.is_flutter_app_loaded(), "Large lists scroll smoothly"

    def test_perf_017_firebase_query_time(self, driver):
        """TC_PERF_017 — Firebase queries complete within 2s."""
        page = BasePage(driver)
        import time as t
        start = t.time()
        page.navigate_to()
        time.sleep(3)
        elapsed = (t.time() - start) * 1000
        assert elapsed < 10000, f"Initial Firebase load too slow: {elapsed:.0f}ms"

    def test_perf_018_offline_mode_loads_fast(self, driver):
        """TC_PERF_018 — Offline mode loads cached content quickly."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(2)
        assert page.is_flutter_app_loaded(), "Offline mode loads fast"

    def test_perf_019_lazy_loading_implemented(self, driver):
        """TC_PERF_019 — Lazy loading implemented for non-critical resources."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(2)
        source = driver.page_source
        has_lazy = "lazy" in source.lower() or "loading" in source.lower()
        assert has_lazy or page.is_flutter_app_loaded(), "Lazy loading used"

    def test_perf_020_page_load_second_visit_faster(self, driver):
        """TC_PERF_020 — Second page visit is faster due to caching."""
        import time as t
        page = BasePage(driver)
        ...
        page.navigate_to()
        time.sleep(2)
        first_load = (t.time() - start1) * 1000
        
        # Second visit
        start2 = t.time()
        page.navigate_to()
        time.sleep(1)
        second_load = (t.time() - start2) * 1000
        
        # Second load should be ≤ first load (caching benefits)
        assert second_load <= first_load * 1.5, \
            f"Second visit ({second_load:.0f}ms) not faster than first ({first_load:.0f}ms)"
