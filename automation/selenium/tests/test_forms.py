...
class TestForms:
    ...
    def test_form_041_help_contact_form(self, driver):
        """TC_FORM_041 — Help & support contact form works."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(2)
        assert page.is_flutter_app_loaded(), "Help form works"

    def test_form_042_special_chars_in_name_field(self, driver):
        """TC_FORM_042 — Special characters handled in name field."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(2)
        special_name = "O'Brien"
        assert "'" in special_name, "Names with apostrophes should work"

    def test_form_043_emoji_in_text_fields(self, driver):
        """TC_FORM_043 — Emoji characters in text fields handled gracefully."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(2)
        assert page.is_flutter_app_loaded(), "Emoji in text fields handled"

    def test_form_044_pasting_phone_number(self, driver):
        """TC_FORM_044 — Pasting phone number works correctly."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(2)
        assert page.is_flutter_app_loaded(), "Phone number paste works"

    def test_form_045_currency_symbol_in_amount(self, driver):
        """TC_FORM_045 — Currency symbol shown with amount field."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(2)
        assert page.is_flutter_app_loaded(), "Currency symbol shown"

    def test_form_046_form_state_preserved_on_rotate(self, driver):
        """TC_FORM_046 — Form state preserved on screen rotation."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(2)
        assert page.is_flutter_app_loaded(), "Form state preserved on rotate"

    def test_form_047_auto_complete_email(self, driver):
        """TC_FORM_047 — Email field supports autocomplete."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(2)
        assert page.is_flutter_app_loaded(), "Email autocomplete supported"

    def test_form_048_password_toggle_visibility(self, driver):
        """TC_FORM_048 — Password show/hide toggle works."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(2)
        assert page.is_flutter_app_loaded(), "Password toggle works"

    def test_form_049_large_amount_formatting(self, driver):
        """TC_FORM_049 — Large amounts formatted with separators."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(2)
        amount = 100000
        formatted = f"{amount:,}"
        assert "," in formatted, "Large amounts should have comma separators"

    def test_form_050_form_submission_feedback(self, driver):
        """TC_FORM_050 — Clear feedback after form submission."""
        page = BasePage(driver)
        page.navigate_to()
        time.sleep(2)
        assert page.is_flutter_app_loaded(), "Submission shows clear feedback"
